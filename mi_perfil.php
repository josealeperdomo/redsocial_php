...
if(!empty($_SESSION['id'])){
    ...
    $stmt->bind_result($id,$nombre_usuario,$email,$password,$rol,$fechad,$fechaa);
    $stmt->fetch();
    ...
    if(!empty($_POST['submit'])){
        $contenido = $_POST['publicacion'];

        if(preg_match('/^.*\S.*$/', $contenido)){
            $stmt = $conn->prepare("INSERT INTO publicaciones (id_usuario, contenido) VALUES (?, ?)");
    
            $stmt->bind_param("is", $_SESSION["id"], $contenido);
            
            $stmt->execute();
    
            $stmt->close();
        }else{
            $publicacionadv = "Texto de publicacion invalido"; 
        }
    }
...
<body>
    ...
    <main class="h-full min-h-[80vh] flex flex-col items-center justify-center p-10 gap-4 text-[#ffffff] bg-[#4DB6AC]">
        <section class="flex flex-col justify-center items-center gap-3">
            ...
            <button class="p-3 bg-[#6ED1C0] rounded-md"><a href="eliminar_perfil.php?id=<?php echo"$id" ?>">Eliminar Perfil<a></button>           
            <button class="p-3 bg-[#6ED1C0] rounded-md"><a href="editar_perfil.php">Editar Perfil<a></button>
        </section>
        <section class="flex flex-col justify-center items-center gap-3">
            <h3 class="text-3xl">Escribe una publicacion</h3>

            <form class="flex flex-col items-center justify-center" action="mi_perfil.php" method="post">
                <textarea class="bg-teal-700 w-80 p-2" name="publicacion" id="publicacion" rows="5" style="resize: none; font-size: 14px;"></textarea>
                </br>
                <input class="p-3 bg-[#6ED1C0] rounded-md" type="submit" name="submit" id="submit" value="Publicar">
            </form>
            <h3><?php echo $publicacionadv; ?></h3>
        </section>
        <section  class="flex flex-col justify-center items-center gap-3">
            ...
            <?php     
                ...
                if ($resultado->num_rows > 0) {
                    while ($fila = $resultado->fetch_assoc()) {
                        ...
                        echo "<button class='p-1 bg-[#6ED1C0] rounded-md'><a href='borrar_publicacion.php?id=".$fila['id'] . "'>Borrar publicacion</a></button>";
                        echo "<button class='p-1 bg-[#6ED1C0] rounded-md'><a href='editar_publicacion.php?id=" . $fila['id'] . "'>Editar publicacion</a></button>";
