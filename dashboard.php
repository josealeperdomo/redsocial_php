...
<header class="bg-teal-700 flex justify-between p-10 text-white items-center">
    ...
    <main class="h-full min-h-[80vh] flex flex-col items-center justify-center p-10 gap-4 text-[#ffffff] bg-[#4DB6AC]">
        <section class="flex flex-col justify-center items-center gap-3">
            ...
            <?php     
                $stmt = $conn->prepare("SELECT publicaciones.id, publicaciones.contenido, publicaciones.fecha_creacion, publicaciones.fecha_actualizacion, usuarios.nombre_usuario FROM publicaciones INNER JOIN usuarios ON publicaciones.id_usuario = usuarios.id ORDER BY publicaciones.id DESC;");
